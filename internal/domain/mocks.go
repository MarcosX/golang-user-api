...
var UserSample, _ = db.NewUser("0", "User", "user@email.com", "pass")
...
func (u *mockUserRepository) CreateUser(name string, email string, password string) (*db.User, error) {
	...
	u.lastId += 1
	user, _ := db.NewUser(fmt.Sprintf("%d", u.lastId), name, email, password)
...
func (u *mockUserRepository) UpdateUser(id string, name string, email string, password string) (*db.User, error) {
	...
	user, _ := db.NewUser(id, name, email, password)
	u.users[id] = user
	return user, nil
}
